...
/*
 * See the comment in ffa_notif_domain_init() below on how the value are
 * assigned.
 */
    ...
    do {
        ...
        for ( n = 0; n < list_count; n++ )
        {
            ...
            if ( d )
            {
                 ...
                 */
                vgic_inject_irq(d, d->vcpu[0], ctx->notif.np_intid, true);
...
bool ffa_notif_domain_init(struct domain *d)
{
     ...
     * the secure world.
     */
    ctx->notif.np_intid = FFA_NOTIF_PEND_INTR_ID;
    ctx->notif.sr_intid = FFA_SCHEDULE_RECV_INTR_ID;
